...
const createZalopayOrder = async (bookingId) => {
  ...
  if (!booking.showtime || !booking.showtime.movie || !booking.showtime.cinema) {
    throw new Error(
      'Booking data is incomplete. Please ensure showtime, movie, and cinema are populated.'
    );
  }
  ...
  const description =
    `Thanh toán vé xem phim - ${movieTitle} tại ${cinemaName} - Suất chiếu: ${startTime}`.trim();
...
const handleCallback = async (dataStr, reqMac) => {
  try {
    ...
    // Cập nhật booking thành công nguyên tử (chỉ update nếu pending hoặc expired)
    ...
    if (!confirmedBooking) {
      console.log('Booking already processed by concurrent request');
      return { return_code: 1, return_message: 'already processed' };
    }

    // Trừ số ghế khả dụng của suất chiếu
    const showtimeId = confirmedBooking.showtime?._id || confirmedBooking.showtime;
    if (showtimeId) {
      await Showtime.findByIdAndUpdate(showtimeId, {
        $inc: { availableSeats: -confirmedBooking.seat.length },
      });
    }

    await seatHold.releaseSeats(showtimeId, confirmedBooking.seat);
...
const handleMomoCallback = async (callbackData) => {
  try {
    ...
    const generatedSignature = CryptoJS.HmacSHA256(rawSignature, secretKey).toString(
      CryptoJS.enc.Hex
    );
    ...
    if (resultCode === 0) {
      ...
      if (amount !== undefined && Number(amount) !== booking.totalPrice) {
        console.error('Momo amount mismatch:', {
          bookingAmount: booking.totalPrice,
          callbackAmount: amount,
        });
        return { statusCode: 400, message: 'Amount mismatch' };
      }
      ...
      if (confirmedBooking) {
        ...
        await seatHold.releaseSeats(showtimeId, confirmedBooking.seat);
        console.log('Momo thanh toan thanh cong');
      }
    }
