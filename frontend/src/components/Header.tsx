      ...
      <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-4">
        <div className="flex items-center gap-8">
          <Link to="/" className="flex items-center gap-3 text-white hover:text-primary transition-colors">
            ...
            <h2 className="text-white text-xl font-bold leading-tight tracking-[-0.015em] hidden sm:block">ABC Cinema</h2>
          </Link>
          {/* Desktop Search */}
          <div className="hidden md:flex flex-col min-w-[240px]">
            <form onSubmit={handleSearch} className="flex w-full flex-1 items-stretch rounded-lg h-10 bg-[#232f48] group focus-within:ring-2 focus-within:ring-primary/50 transition-all">
        ...
        <div className="flex items-center gap-4 lg:gap-8">
          ...
          {user && (
            <Link to="/bookings" className="hidden md:block text-white text-sm font-medium hover:text-primary transition-colors">
              Lịch sử đặt vé
            </Link>
          )}
          {/* Theme Toggle */}
          <button
            onClick={toggleTheme}
            className="flex items-center justify-center rounded-lg h-9 w-9 bg-[#232f48] text-white hover:bg-[#324467] transition-colors"
