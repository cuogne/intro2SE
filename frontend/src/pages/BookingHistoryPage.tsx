...
import { getBookingHistory, type Booking } from '../services/bookingService';
        ...
        <div className="space-y-4">
          {bookings.map((booking) => {
            ...
            return (
              <div
                key={booking._id}
                className="bg-white dark:bg-[#1a2332] border border-gray-200 dark:border-[#324467] rounded-xl p-6 hover:bg-gray-50 dark:hover:bg-[#232f48] transition-colors cursor-pointer"
                onClick={() => {
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-2 text-gray-600 dark:text-text-secondary">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">{booking.showtime.cinema.address}</span>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      {booking.paidAt && (
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            Thanh toán: {new Date(booking.paidAt).toLocaleString('vi-VN')}
                          </span>
                        </div>
                      )}
                      {isPast && booking.status === 'confirmed' && (
                        <p className="text-sm text-yellow-400 mt-2">Đã xem</p>
                      )}
                    </div>
                  </div>
