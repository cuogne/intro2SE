...
const HomePage: React.FC = () => {
    ...
    useEffect(() => {
        const loadMovies = async () => {
            setLoading(true);
            try {
                const [nowShowingRes, comingSoonRes] = await Promise.all([fetchMovies("Now Showing"), fetchMovies("Coming Soon")]);
    ...
    const handleWatchTrailer = (trailerLink?: string, title?: string) => {
        if (trailerLink) {
            setSelectedTrailer({ link: trailerLink, title });
            setShowTrailer(true);
        }
    };
        ...
        <div className="flex-grow flex flex-col items-center">
            ...
            {heroMovie && (
                <div className="w-full max-w-[1440px] px-4 md:px-10 py-6">
                    <div
                        className="flex min-h-[480px] md:min-h-[560px] flex-col gap-6 bg-cover bg-center bg-no-repeat rounded-xl md:rounded-2xl items-start justify-end px-6 pb-12 md:px-12 md:pb-16 relative overflow-hidden group"
                        style={{
                            backgroundImage: `linear-gradient(to top, rgba(16, 22, 34, 1) 0%, rgba(16, 22, 34, 0.6) 50%, rgba(16, 22, 34, 0.1) 100%), url(${
                                heroMovie.posterImg || "https://via.placeholder.com/1440x560"
                            })`,
                        }}
                    >
                        <div className="z-10 flex flex-col gap-4 max-w-2xl animate-fade-in-up">
                            ...
                            <h1 className="text-white text-4xl md:text-6xl font-black leading-tight tracking-tight drop-shadow-xl">{heroMovie.title}</h1>
                            <p className="text-gray-200 text-base md:text-lg font-normal line-clamp-2 max-w-xl drop-shadow-md">{heroMovie.description || "Một kiệt tác điện ảnh không thể bỏ lỡ."}</p>
                            <div className="flex flex-wrap gap-4 mt-4">
                                ...
                                {heroMovie.trailerLink && (
                                    <button
                                        onClick={() => handleWatchTrailer(heroMovie.trailerLink, heroMovie.title)}
            ...
            <div className="w-full max-w-[1440px] px-4 md:px-10 mt-4">
                <div className="border-b border-gray-300 dark:border-[#232f48] flex gap-8 mb-8">
                    <button onClick={() => setActiveTab("now")} className="relative pb-4 px-2 cursor-pointer group">
                        <p className={`text-lg font-bold transition-colors text-black ${activeTab === "now" ? "text-primary" : "text-gray-600"} group-hover:text-primary`}>Phim Đang Chiếu</p>
                        {activeTab === "now" && <div className="absolute bottom-0 left-0 h-[3px] w-full bg-primary rounded-t-full"></div>}
                    </button>
                    <button onClick={() => setActiveTab("soon")} className="relative pb-4 px-2 cursor-pointer group">
                        <p className={`text-lg font-medium transition-colors text-black ${activeTab === "soon" ? "text-primary" : "text-gray-600"} group-hover:text-primary`}>Phim Sắp Chiếu</p>
                        {activeTab === "soon" && <div className="absolute bottom-0 left-0 h-[3px] w-full bg-primary rounded-t-full"></div>}
                        {activeTab !== "soon" && (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-6 gap-y-10">
                    {displayMovies.map((movie) => (
                        <div key={movie._id} className="group flex flex-col gap-3">
                            <div className="relative aspect-2/3 w-full overflow-hidden rounded-xl bg-gray-200 dark:bg-[#232f48]">
                                ...
                                <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-3 p-4 z-10">
                                    ...
                                    {movie.trailerLink && (
                                        <button
                                            onClick={(e) => {
                                                e.preventDefault();
                                                e.stopPropagation();
                                                handleWatchTrailer(movie.trailerLink, movie.title);
                                            }}
                                    ...
                                    <button
                                        ...
                                        className="w-full py-3 bg-white/20 backdrop-blur-sm text-white font-bold rounded-lg border border-white/30 hover:bg-white/30 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 delay-150"
                                    >
                                        Chi Tiết
                                    </button>
                                </div>
                                ...
                                <div className="absolute top-3 left-3 flex gap-2">
                                    <span className="bg-orange-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">{getAgeRating(movie)}</span>
                                </div>
                            ...
                            <div>
                                <h3 className="text-gray-900 text-base font-bold leading-tight truncate group-hover:text-primary transition-colors">{movie.title}</h3>
            ...
            {/* Promotions Section */}
            <div className="w-full bg-gray-100 rounded-lg dark:bg-[#161d2b] py-12 mt-8">
            ...
            {/* Trailer Modal */}
            <TrailerModal isOpen={showTrailer} onClose={() => setShowTrailer(false)} trailerLink={selectedTrailer.link} movieTitle={selectedTrailer.title} />
        </div>
    );
};
