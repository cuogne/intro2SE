...
import { Clock, Calendar, MapPin, Play, Ticket } from "lucide-react";
...
const MovieDetailPage: React.FC = () => {
    ...
    if (loading) return <div className="text-center py-20 text-white dark:text-white">Đang tải dữ liệu...</div>;
    if (!movie) return <div className="text-center py-20 text-white dark:text-white">Không tìm thấy phim</div>;
    ...
    return (
        <div className="container mx-auto px-4 py-8 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                <div className="md:col-span-1">
                    <div className="rounded-lg overflow-hidden shadow-lg border border-gray-200 dark:border-[#324467]">
                        <img src={movie.posterImg} alt={movie.title} className="w-full h-auto object-cover aspect-2/3" />
                    </div>
                </div>

                <div className="md:col-span-2 space-y-6">
                    <h1 className="text-4xl font-bold uppercase tracking-tight text-gray-900 dark:text-white">{movie.title}</h1>
                    <div className="flex flex-wrap gap-4 text-sm text-gray-700 dark:text-text-secondary">
                        ...
                        <div className="flex items-center gap-1 bg-gray-100 dark:bg-[#232f48] px-3 py-1 rounded">Thể loại: {movie.genre.join(", ")}</div>
                    </div>

                    <div>
                        <h3 className="font-bold text-lg mb-2 text-gray-900 dark:text-white">MÔ TẢ</h3>
                        <p className="text-gray-600 dark:text-text-secondary leading-relaxed text-justify">{movie.description || "Chưa có mô tả."}</p>
                    </div>
            ...
            <div id="schedule-section" className="mt-12">
                <h2 className="text-2xl font-bold mb-6 border-l-4 border-primary dark:border-primary pl-3 text-gray-900 dark:text-white">Lịch chiếu</h2>

                {/* Selector Ngày Cố định */}
                <div className="flex gap-3 overflow-x-auto pb-4 mb-8 p-2 scrollbar-hide">
                ...
                <div className="space-y-6">
                    {theaterGroups.length > 0 ? (
                        theaterGroups.map((group, idx) => (
                            <div key={idx} className="bg-white dark:bg-[#1a2332] border border-gray-200 dark:border-[#324467] rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
                                <div className="mb-4">
                                    <h3 className="font-bold text-lg text-gray-900 dark:text-white">{group.cinema.name}</h3>
            ...
            {/* --- MODAL TRAILER --- */}
            <TrailerModal isOpen={showTrailer} onClose={() => setShowTrailer(false)} trailerLink={movie?.trailerLink} movieTitle={movie?.title} />
        </div>
    );
};
