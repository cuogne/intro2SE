...
import { fetchShowtimeDetail, type ShowtimeDetail } from '../services/showtimeService';
...
const SeatSelectionPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadShowtime = async () => {
      setLoading(true);
      const data = await fetchShowtimeDetail(showtimeId);
      if (data) {
        setShowtime(data);
      } else {
  ...
  // Nhóm ghế theo hàng
  const groupedSeats: { [key: string]: typeof showtime.seats } = {};
  if (showtime) {
