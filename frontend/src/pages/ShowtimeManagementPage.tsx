...
const ShowtimeManagementPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const { docs, pagination } = await fetchShowtimes(
        page,
        PAGE_SIZE,
        filters
      );
  ...
  const [editing, setEditing] = useState<ShowtimeListItem | undefined>(
    undefined
  );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-500 dark:text-text-secondary text-sm">
            Danh sách các suất chiếu phim trong tuần
          </p>
        </div>
        <button
          onClick={() => openAdd()}
        ...
        <div className="flex flex-col md:flex-row items-end gap-4">
          <label className="flex flex-col w-full md:flex-1">
            ...
            <div className="relative">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-text-secondary text-[20px]">
                search
              </span>
              <input
                value={search}
      ...
      <div className="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-border-dark shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-[#111318] border-b border-slate-200 dark:border-border-dark">
                  <th className="p-4 text-xs font-semibold text-slate-500 dark:text-text-secondary uppercase tracking-wider">
                    Tên phim
                  </th>
                  <th className="p-4 text-xs font-semibold text-slate-500 dark:text-text-secondary uppercase tracking-wider">
                    Giá vé
                  ...
                  <th className="p-4 text-xs font-semibold text-slate-500 dark:text-text-secondary uppercase tracking-wider text-right">
                    Hành động
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 dark:divide-border-dark">
                {current.map((s) => (
                  <tr
                    key={s._id}
                    className="hover:bg-slate-50 dark:hover:bg-border-dark/50 transition-colors group"
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-slate-900 dark:text-white truncate max-w-120">
                            {s.movie?.title}
                          </span>
                          <span className="text-xs text-slate-500 dark:text-text-secondary">
                            {minuteToDurationStr(s.movie?.minutes)}
                          </span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex flex-col">
                        <span className="text-sm truncate text-slate-900 dark:text-white font-medium">
                          {formatISOToVN(s.startTime)}
                        </span>
                        <span className="text-sm truncate text-slate-900 dark:text-white font-medium">
                          {formatISOToVN(s.endTime)}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      {statusBadge(
                        computeStatus(s.startTime, s.endTime, s.movie?.minutes)
                      )}
                    </td>
                ...
                {current.length === 0 && (
                  <tr>
                    <td
                      className="p-4 text-sm text-slate-500 dark:text-text-secondary"
                      colSpan={5}
                    >
                      Không có kết quả
                    </td>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-border-dark bg-slate-50 dark:bg-[#111318]">
          <div className="text-sm text-slate-500 dark:text-text-secondary">
            Hiển thị {currentStart <= totalDocs ? currentStart : 0} -{" "}
            {currentEnd <= totalDocs ? currentEnd : 0} của {totalDocs} suất
            chiếu
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <AntPagination
                current={page}
                pageSize={PAGE_SIZE}
                total={paginationInfo?.totalDocs ?? 0}
                onChange={goToPage}
