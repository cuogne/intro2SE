 ...
 */
export const fetchShowtimes = async (page = 1, limit = 10, filters: { movieId?: string; cinemaId?: string; date?: string } = {}): Promise<ShowtimesListResponse> => {
    try {
        ...
        const docs: ShowtimeListItem[] = Array.isArray(payload) ? (payload as ShowtimeListItem[]) : Array.isArray(payload.docs) ? payload.docs : [];
        ...
        const pagination: Pagination = payload.pagination
            ...
            : {
                  ...
                  totalDocs: payload.totalDocs ?? (Array.isArray(docs) ? docs.length : 0),
                  totalPages: payload.totalPages ?? Math.ceil((payload.totalDocs ?? (Array.isArray(docs) ? docs.length : 0)) / (payload.limit ?? limit) || 0),
              };

        return { docs, pagination };
    } catch (error) {
        console.error("Lỗi gọi /v1/showtimes:", error);
        return { docs: [], pagination: { page, limit, totalDocs: 0, totalPages: 0 } };
    }
};
 ...
 */
export const fetchShowtimeById = async (id: string): Promise<ShowtimeDetail | null> => {
    try {
        const res = await api.get(`/v1/showtimes/${id}`);
        const data = res.data?.data ?? res.data;
        // Backend returns { success: true, data: { ... } }
        const doc = Array.isArray(data) ? data.find((d) => d._id === id) ?? null : data ?? null;
 ...
 * }
 */
export const createShowtime = async (payload: CreateShowtimePayload): Promise<ShowtimeDetail | null> => {
    try {
...
export const updateShowtime = async (id: string, payload: Partial<UpdateShowtimePayload>): Promise<ShowtimeDetail | null> => {
    try {
...
export interface CinemaShowtimeGroup {
    ...
    showtimes: { id: string; time: string; price: number; availableSeats: number }[];
}
...
export const fetchShowtimesByMovie = async (movieId: string, date: Date): Promise<CinemaShowtimeGroup[]> => {
    try {
        ...
        const filtered = docs.filter((s) => new Date(s.startTime).toLocaleDateString("en-GB") === targetDateStr && s.movie._id === movieId);
        ...
        filtered.forEach((s) => {
            ...
            if (!groupsMap[cid]) {
                groupsMap[cid] = { cinema: { _id: cid, name: s.cinema.name, address: s.cinema.address }, showtimes: [] };
            }
        ...
        return Object.values(groupsMap).map((g) => ({ ...g, showtimes: g.showtimes.sort((a, b) => a.time.localeCompare(b.time)) }));
    } catch (error) {
